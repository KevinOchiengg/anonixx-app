...
const VideoPlayer = React.memo(
  ({ videoUrl, isActive, viewCount, onMediaPress }) => {
    ...
    useEffect(() => {
      const sub = inlinePlayer.addListener(
        'playingChange',
        ({ isPlaying: playing }) => {
    ...
    return (
      ...
      >
        ...
        <Animated.View
          pointerEvents="none"
          style={[styles.videoCenter, { opacity: overlayOp }]}
        >
          ...
          <View style={styles.videoMetaRow}>
            {isPlaying && (
              ...
              >
                ...
                <Text style={styles.videoMutedText}>
                  {isMuted ? 'unmute' : 'mute'}
                </Text>
              </TouchableOpacity>
            )}
...
const PollCard = React.memo(({ poll, postId, isAuthenticated, onVote }) => {
  ...
  const hasVoted =
    poll.voted_option !== null && poll.voted_option !== undefined;
          ...
          if (hasVoted) {
            ...
            return (
              <View key={i} style={styles.pollResultRow}>
                <View
                  style={[
                    styles.pollResultBar,
                    isChosen && styles.pollResultBarChosen,
                  ]}
                >
                  <Animated.View
                    style={[
                  ...
                  <Text
                    style={[
                      ...
                      isChosen && styles.pollResultTextChosen,
                    ]}
                    numberOfLines={1}
                  >
                    {opt.text}
                ...
                <Text
                  style={[
                    styles.pollPercent,
                    isChosen && styles.pollPercentChosen,
                  ]}
                >
                  {opt.percent ?? 0}%
                </Text>
...
function CalmPostCard({
    ...
    async (optionIndex) => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE_URL}/api/v1/posts/${post.id}/vote`,
          {
            method: 'POST',
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        if (res.ok) {
          ...
          showToast({
            type: 'error',
            message: data.detail || 'Could not submit vote.',
          });
        }
      } catch {
  ...
  // ── Copy Link — HTTPS link ──────────────────────────────────
  const handleCopyLink = useCallback(async () => {
    setMenuVisible(false);
    try {
      await Clipboard.setStringAsync(
        `${BASE_URL}/api/v1/posts/${post.id}/open`
      );
        ...
        <DoubleTapLike onDoubleTap={handleDoubleTap}>
          ...
          >
            ...
            {(post.video_url || post.audio_url) && (
              <Text style={styles.hint}>swipe through confessions ↑</Text>
            )}
                ...
                >
                  <Animated.View style={{ transform: [{ scale: scaleAnim }] }}>
                    <Heart
                      ...
                      fill={liked ? T.primary : 'none'}
                    />
                  </Animated.View>
                  <Text
                    style={[styles.actionCount, liked && { color: T.primary }]}
                  >
                    {likesCount}
                  </Text>
                </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  videoGradient: {
    ...
    shadowColor: '#000',
    shadowOffset: { width: 0, height: -rs(40) },
    shadowOpacity: 0.8,
    shadowRadius: rs(40),
  },
  videoCenter: {
...
const areEqual = (prev, next) =>
  ...
  (prev.activeVideoId === prev.post.id) ===
    (next.activeVideoId === next.post.id);
