...
export default function ChatScreen({ route, navigation }) {
  ...
  const sendMessage = async () => {
    ...
    setMessages((prev) => [...prev, {
      id: tempId, content, is_own: true, is_read: false, created_at: new Date().toISOString()
    }]);
    setInputText('');
    scrollToBottom();
    ...
    try {
      ...
      if (res.ok) {
        if (accept && data.other_user) {
          setChatInfo((prev) => ({
            ...prev,
            reveal_status: 'accepted',
            revealed_other: data.other_user
          }));
        } else {
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <View style={[styles.headerAvatar, { backgroundColor: (otherAvatarColor || '#FF634A') + '22', borderColor: (otherAvatarColor || '#FF634A') + '55' }]}>
        ...
        >
          <Text style={styles.revealBtnText}>
            {chatInfo?.reveal_status === 'accepted' ? '✨' : chatInfo?.reveal_status === 'pending' ? '👁' : '👁'}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      {chatInfo?.revealed_other && (
        <View style={styles.revealedBanner}>
          <Text style={styles.revealedBannerText}>
            ✨ {chatInfo.revealed_other.username}
          </Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  limitBannerWarn: {
    ...
    borderBottomWidth: 1,
    borderBottomColor: 'rgba(255,99,74,0.12)',
    alignItems: 'center',
  },
  limitBannerWarnText: {
    color: THEME.textSecondary,
